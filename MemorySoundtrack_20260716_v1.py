...
with tab1:
    ...
    if uploaded_files:
        ...
        # Display image previews in a column grid
        cols = st.columns(min(len(files_to_process), 5))
        for i, file in enumerate(files_to_process):
            with cols[i]:
                img = Image.open(file)
                st.image(img, use_container_width=True, caption=f"Memory #{i+1}")
        ...
        if st.button("🧠 Analyze Memories", key="btn_analyze"):
            ...
            if client:
                ...
                for idx, file in enumerate(files_to_process):
                    progress_bar.progress((idx + 1) / len(files_to_process), text=f"Processing Photo #{idx+1}...")
