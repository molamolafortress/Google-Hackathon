...
logger = logging.getLogger("MemorySoundtrackWeb_v5")

# Load environment variables from .env file
load_dotenv()

# ----------------------------------------------------
# Pydantic Schema for Unified Prompt Synthesis (v5)
# ----------------------------------------------------
...
# ----------------------------------------------------
st.set_page_config(
    page_title="Memory Soundtrack - Swift Personalized Edition (v5)",
    page_icon="🎵",
    layout="wide",
...
<div style='text-align: center; padding: 10px 0;'>
    ...
    <p style='color:#94a3b8; font-size:0.85rem; margin-top:5px;'>Ultimate Personalized Edition v5</p>
</div>
...
def analyze_single_image(client, idx, file, gemini_model):
    ...
    try:
        ...
        prompt = (
            ...
            "⚠️ 극도로 중요한 주의사항: 분석 본문 and 제안에 '콜드플레이', '지브리', 'BTS', '아이유', '신카이 마코토' 등 어떠한 특정 아티스트명, 밴드명, 작곡가명, 애니메이션, 혹은 플랫폼 상표명(Suno, Udio, 유튜브, 스포티파이 등)도 언급하는 것을 완전히 금지합니다. 오직 보편적인 음악 묘사 단어만을 사용해야 합니다.\n\n"
...
# ----------------------------------------------------
# 5. Image Transformation Helper (Timeline Photo #1 to Square Album Cover)
# ----------------------------------------------------
def get_first_image_as_cover_bytes(first_file) -> bytes:
    """
    Opens the first chronologically ordered slide photo, square center-crops it,
    resizes it to a premium 512x512 PNG, and returns the raw bytes for the spinning LP.
    """
    try:
        ...
        logger.error(f"Failed to convert timeline Slide #1 to album cover: {str(e)}")
        # Ultimate fallback: Create an elegant dark gradient image
        fallback_img = Image.new("RGB", (512, 512), "#0f0f1b")
        buf = io.BytesIO()
        fallback_img.save(buf, format="PNG")
        return buf.getvalue()

# ----------------------------------------------------
# 6. Main Interface Header
# ----------------------------------------------------
...
# ----------------------------------------------------
# 7. Step 1: Upload & Timeline Configuration
# ----------------------------------------------------
...
# ----------------------------------------------------
# 8. Step 2: One-Click Master Execution (Streamlined 3-Stage Pipeline)
# ----------------------------------------------------
if uploaded_files and final_files_to_process:
    ...
    st.markdown("### 💿 2. Create Soundtrack & Album Cover")
    st.write("Clicking the button below will analyze your memories in parallel, synthesize safe prompts, compose your soundtrack via Lyria, and automatically personalize your first photo into the album cover art.")
    
    if st.button("🎶 나의 추억 앨범 생성하기 (One-Click Generation)", key="btn_one_click_generate"):
        client = get_client()
        if client:
            # Stage 1/3: Parallel Multi-Agent Image Story Analysis
            st_stage1_progress = st.empty()
            st_stage1_progress.info("⚡ [Stage 1/3] 멀티 에이전트(Multi-Agent)를 기동하여 모든 이미지 분석을 초고속 동시 병렬 진행하는 중입니다...")
            
            analyses_results = []
            try:
                # Execute Gemini storyteller in parallel threads
                ...
                # Sort in correct chronological index order
            ...
            # Stage 2/3: Prompt Synthesis & Storyline Summarization
            st_stage2_status = st.empty()
            with st_stage2_status.container():
                st.info("🧠 [Stage 2/3] Synthesizing stories into safe production soundtrack prompts...")
                combined_context = "\n\n".join([
            ...
            # Stage 3/3: Lyria Soundtrack Composition & Direct Personalized Cover Binding
            st_stage3_status = st.empty()
            audio_success = False
            with st_stage3_status.container():
                st.info("🎶 [Stage 3/3] Composing single soundtrack with DeepMind Lyria & Binding Slide #1 as album art...")
                current_prompt = st.session_state['soundtrack_prompt']
                
                # Execute Lyria with self-healing retry pipeline
                ...
                # If audio is successful, bind Slide #1 as the album art instantly (Bypasses Imagen entirely!)
                if audio_success:
                    st.info("🖼️ Personalizing album art with your first memory photo...")
                    # Slide #1 is located at index 0 of final_files_to_process
                    first_uploaded_file = final_files_to_process[0]
                    cover_bytes = get_first_image_as_cover_bytes(first_uploaded_file)
...
# ----------------------------------------------------
# 9. Step 3: Complete Album Playback & Slide Deck
# ----------------------------------------------------
if st.session_state['album_generated'] and st.session_state['audio_bytes'] and st.session_state['cover_bytes']:
    ...
    with st.expander("⚙️ 프롬프트 수정 및 재작곡 (Advanced Prompt Editor)", expanded=False):
        st.info("You can fine-tune the synthesized prompts and summary manually and re-generate the album single. Your custom first-photo cover art remains preserved.")
        ...
        if st.button("🔄 수정된 프롬프트로 재작곡하기 (Re-compose Single)", key="btn_recompose"):
            ...
            if client:
                ...
                if audio_success:
                    st.success("🎉 Single soundtrack successfully re-composed!")
                    st.rerun()
